...
//				tutButton = Instantiate(tutorialButton, new Vector3(0, -pos.y / 2f), Quaternion.identity) as Transform;
		...
		//THIS IS CHARACTER SELECT
			...
			if (player1Select == 1) GUI.Box (new Rect(-Screen.width / 40, Screen.height / 17, MMT.width, MMT.height), MMT, style);
			...
			if (player2Select == 1) GUI.Box (new Rect(Screen.width * 1.03f / 2, Screen.height / 17, MMT.width, MMT.height), MMT, style);
